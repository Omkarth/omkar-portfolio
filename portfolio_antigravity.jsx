...
// ===== CYBERLLM INLINE DEMO =====
const CYBER_RESPONSES = {
  "What is SQL injection?": "SQL injection occurs when an attacker inserts malicious SQL into input fields that get incorporated into database queries, allowing the execution of arbitrary commands in the application's database. Prevention: use parameterized queries and prepared statements for user input; proper validation and sanitization of incoming data; least privilege database roles; and access management controls.",
  "How do I respond to a ransomware incident?": "Steps: 1) Contain the threat — disconnect affected systems from network to stop lateral movement; 2) Assess scope — isolate impacted systems, preserve evidence, identify the ransomware variant; 3) Restore from backups — verify backup integrity, restore from offline copies or snapshots; 4) Report and notify — follow internal/external notification rules, contact authorities if required; 5) Post-incident — document lessons learned, update IR plan, improve defenses.",
  "Explain zero trust architecture": "A zero trust architecture operates on the principle of 'never trust, always verify.' Every access request is continuously verified against identity, device health, and context regardless of network location. Key components: micro-segmentation to limit lateral movement, identity-aware proxies, multi-factor authentication for all resources, just-in-time access provisioning, and encryption of all data in transit.",
  "What is the MITRE ATT&CK framework?": "MITRE ATT&CK is a knowledge base of adversary tactics, techniques, and procedures (TTPs) based on real-world observations. It covers the full attack lifecycle from Initial Access through Exfiltration. Security teams use it for threat hunting hypothesis development, detection engineering, red team planning, and gap analysis.",
  "Explain Kerberoasting": "Kerberoasting is an Active Directory attack where an attacker requests Kerberos service tickets (TGS) for accounts with Service Principal Names (SPNs). These tickets are encrypted with the service account's password hash, which can be cracked offline. Defense: use strong passwords for service accounts, implement Group Managed Service Accounts (gMSA), monitor for anomalous TGS requests.",
};
const CYBER_SUGGESTIONS = Object.keys(CYBER_RESPONSES);

function CyberLLMInlineDemo() {
  ...
  const findMatch = (q) => {
    ...
    for (const key of Object.keys(CYBER_RESPONSES)) {
      const score = ql.split(/\s+/).filter(w => key.toLowerCase().includes(w)).length;
      if (score > bestScore) { bestScore = score; best = key; }
    }
    return bestScore >= 2 ? CYBER_RESPONSES[best] : "Try one of the suggested questions for the best demo experience. This model specializes in cybersecurity topics.";
  };
  ...
  const ask = async (q) => {
    ...
    setMsgs(prev => [...prev, { role: 'assistant', content: findMatch(question) }]);
    setGenerating(false);
  };

  return (
    <div style={{ background: '#0a0a12', border: '1px solid #1a1a2a', borderRadius: 10, overflow: 'hidden' }}>
      {/* Header */}
      <div style={{ padding: '12px 16px', borderBottom: '1px solid #1a1a2a', display: 'flex', alignItems: 'center', gap: 8 }}>
        <div style={{ width: 24, height: 24, borderRadius: 6, background: 'linear-gradient(135deg, #00ff88, #00cc6a)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 800, color: '#000' }}>C</div>
        <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 12, fontWeight: 700, color: '#e0e0e8' }}>CyberLLM</span>
        <span style={{ fontSize: 10, color: '#00ff88', background: '#00ff8815', padding: '2px 8px', borderRadius: 10, fontFamily: "'Space Mono', monospace" }}>DEMO</span>
      </div>

      {/* Suggestions */}
      <div style={{ padding: '10px 16px', display: 'flex', gap: 6, flexWrap: 'wrap', borderBottom: '1px solid #1a1a2a' }}>
        {CYBER_SUGGESTIONS.slice(0, 4).map(s => (
          <button key={s} onClick={() => ask(s)} style={{
            fontFamily: "'Space Mono', monospace", fontSize: 9, padding: '5px 10px',
            background: '#10101a', border: '1px solid #1a1a2a', borderRadius: 6,
            color: '#888898', cursor: 'pointer', transition: 'all 0.2s'
          }}
          onMouseEnter={e => { e.target.style.borderColor = '#00ff88'; e.target.style.color = '#00ff88'; }}
          onMouseLeave={e => { e.target.style.borderColor = '#1a1a2a'; e.target.style.color = '#888898'; }}>
      ...
      <div style={{ height: 220, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
        {msgs.length === 0 && (
          <div style={{ textAlign: 'center', color: '#444460', fontSize: 12, marginTop: 60, fontFamily: "'Space Mono', monospace" }}>
            Ask CyberLLM a cybersecurity question...
          </div>
        )}
        {msgs.map((m, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
            <div style={{
              ...
              background: m.role === 'user' ? 'linear-gradient(135deg, #00ff88, #00cc6a)' : '#10101a',
              color: m.role === 'user' ? '#000' : '#c0c0d0',
              border: m.role === 'user' ? 'none' : '1px solid #1a1a2a',
            }}>{m.content}</div>
          </div>
        ))}
        {generating && (
          <div style={{ display: 'flex', gap: 4, padding: '10px 14px' }}>
            {[0, 1, 2].map(i => (
              <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: '#00ff88', animation: `bounce 1.4s ease infinite ${i * 0.2}s` }} />
            ))}
          </div>
        )}
        <div ref={chatEndRef} />
      ...
      {/* Input */}
      <div style={{ padding: '10px 16px', borderTop: '1px solid #1a1a2a', display: 'flex', gap: 8 }}>
        <input value={inp} onChange={e => setInp(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && ask()}
          placeholder="Ask a cybersecurity question..."
          style={{
            flex: 1, padding: '8px 12px', background: '#10101a', border: '1px solid #1a1a2a',
            borderRadius: 8, color: '#e0e0e8', fontSize: 12, fontFamily: "'Space Mono', monospace", outline: 'none'
          }} />
        <button onClick={() => ask()} disabled={generating} style={{
          padding: '8px 16px', background: generating ? '#1a1a2a' : 'linear-gradient(135deg, #00ff88, #00cc6a)',
          border: 'none', borderRadius: 8, color: '#000', fontWeight: 700, fontSize: 11, cursor: generating ? 'wait' : 'pointer',
  ...
  return (
    <div onClick={onClose} style={{ position:'fixed', inset:0, zIndex:1000, background:'rgba(0,0,0,0.88)', backdropFilter:'blur(25px)', display:'flex', alignItems:'center', justifyContent:'center', padding:20, animation:'fadeIn 0.3s ease' }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:'#12121a', border:'1px solid rgba(255,255,255,0.08)', borderRadius:12, maxWidth:800, width:'100%', maxHeight:'90vh', overflow:'auto' }}>
        ...
        {project.hasDemo && (
          <div style={{ padding:'0 40px', marginBottom:32 }}>
            <h3 style={{ fontFamily:"'Syne',sans-serif", fontSize:16, fontWeight:700, marginBottom:12, display:'flex', alignItems:'center', gap:8 }}><Zap size={16} style={{ color:project.color }}/> Live Demo</h3>
            <CyberLLMInlineDemo />
          </div>
        )}
